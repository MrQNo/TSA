...
object LichessApi:
  @main
  def main(): Unit =
    val session = TournamentAdmin.lichessSession
    val arenaFile = os.pwd / "swiss.txt"
    val arenaTxt: Iterator[String] = os.read(arenaFile).linesIterator
    while arenaTxt.hasNext do
      val arenaString = arenaTxt.next()
      println(arenaString)
      val arenaObject: LichessSwissTournamentListEntry = read(arenaString)
